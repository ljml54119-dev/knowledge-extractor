"""
图片知识点总结器 - 本地服务器
使用前设置环境变量: set ANTHROPIC_API_KEY=sk-ant-...
...
class Handler(http.server.BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        if self.path == "/api/summarize":
            ...
            if not api_key:
                self._send_json(400, {"error": "服务器未配置 ANTHROPIC_API_KEY 环境变量"})
                return
            ...
            try:
                text = call_anthropic(api_key, image_data, mime_type)
